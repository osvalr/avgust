...
--
 ...
 /* Inicio JRaffo 18/09/2015 */
  ...
  --
    ...
    from tron2000.mp_genbdpb_mpe a ,
    (select * from a2000030 where tip_spto = 'RD' and cod_cia = 2) x 

    where 
         a.np = x.num_poliza and
         extract (year from x.fec_actu) = a.anio;
    ...
    where 
        ...
        x.fec_actu = (select max(fec_actu) from a2000030 where tip_spto = 'AN' and cod_cia = 2 and num_poliza = a.np);
  ...
  --
    ...
    --l_cod_modalidad := sf_recupera_cod_modalidad(2,l_reg_bonos.cod_ramo,l_reg_bonos.np);
