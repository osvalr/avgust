...
--
  ...
  l_saldo_medio number(10,2);
  l_estado_pago number(10,2);
  ...
  --
    ...
    where 
        ...
        extract (year from z.fec_efec_spto) = a.anio and
  ...
  BEGIN
    ...
    --
      ...
      --
      INSERT INTO TRON2000.MP_GENBDPB_2015_MPE
...
--Obtenes el saldo medio
select

sum((x.monto * x.peso)) into l_saldo_medio
...
from
    ...
    tip_spto,'AN',
        (cap_spto *-1),
        (select q.cap_amortizado from (select rownum num, cap_amortizado from a2300333 where num_poliza = reg.num_poliza and tip_spto in ('AN','CN') and num_spto_afectado = reg.num_spto) q where  q.num = (x.num - 1))
    ) monto,
    (select round((d.meses_rentables/d.total_meses),2) peso from TA_MESES_RENT_PESO d where d.num_mes = x.mes) peso
    from 
    (
    select 
    rownum as num ,
    num_poliza,
    tip_spto,
    extract (year from fec_efec_spto) as anio, 
    extract (month from fec_efec_spto) as mes, 
    fec_efec_spto ,
    cap_spto,
    cap_amortizado
    from a2300333 
    where num_poliza = reg.num_poliza and tip_spto in ('AN','CN') and num_spto_afectado = reg.num_spto
    ) x
) x

where x.anio = reg.anio;

-- Obtenemos el estado de pago (Completo o No)
select

count(*) into l_estado_pago
...
where x.anio = reg.anio and x.pago_completo = 'S' ;

--------------

   update tron2000.MP_GENBDPB_2015_MPE
    set MCA_PRES = 'S',
         SAL_PRES = l_saldo_medio,
         FEC_PRES = reg.fec_efec_spto
    where np = reg.num_poliza and anio = reg.anio;
    ----
    
    if l_estado_pago = 0 then
    
        FOR rc IN (select * from tron2000.MP_GENBDPB_2015_MPE  where np = reg.num_poliza and anio > reg.anio)
          LOOP
            ...
            where x.anio = rc.anio;

            update tron2000.MP_GENBDPB_2015_MPE
                set MCA_PRES = 'S',
                     SAL_PRES = l_saldo_medio
                where np = reg.num_poliza and anio = rc.anio;
